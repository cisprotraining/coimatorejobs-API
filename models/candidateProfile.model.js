...
createdBy: {
  ...
  currentSalary: {
    type: String,
    // Expanded enum to accept "10 Lakhs" style strings
  },
  expectedSalary: {
    type: String,
    // Expanded enum to accept "15 Lakhs" style strings
  },
  experience: {
    type: String,
    required: [false, 'Experience is required'],
  ...
  educationLevels: {
    type: [String],
    required: [true, 'Education levels are required'],
    // Removed strict enum to allow values like "B.E / B.Tech"
  },
  languages: {
    type: [String],
    required: [true, 'Languages are required'],
  ...
  approvedBy: {
    type: mongoose.Schema.Types.ObjectId,
    ref: 'User', // Changed from 'users' to 'User' for consistency
    default: null
  },
  approvedAt: {
    type: Date,
...
// Post-save hook for resume alerts
candidateProfileSchema.post('save', async function (doc) {
  try {
    const alerts = await ResumeAlert.find({ isActive: true }).populate('employer', 'email');
    for (const alert of alerts) {
